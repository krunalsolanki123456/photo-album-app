...
import {
  ArrowLeft, BookOpen, Camera, Check, ChevronLeft, ChevronRight, Copy, Crown,
  FileText, GripVertical, ImagePlus, LayoutGrid, LogOut, Music, Pause, Play,
  Plus, Save, Share2, Trash2, Type, Upload, User, Volume2, X, ZoomIn, ZoomOut, Move, RotateCcw, Eye, EyeOff
} from 'lucide-react';
import './styles.css';
...
function PageCanvas({ page, photosById, editor=false, onDragStart, onDrop, onRemove, onUpdatePhoto=()=>{}, onUpdatePage=()=>{}, selectedPhoto='', onTapSlot=()=>{} }) {
  ...
  const startPan = (e, photo, idx) => {
    ...
    panRef.current={photoId:photo.id,pointerId:e.pointerId,startX:e.clientX,startY:e.clientY,startFocusX:adjust.focusX,startFocusY:adjust.focusY,width:Math.max(1,rect.width),height:Math.max(1,rect.height)};
    e.currentTarget.setPointerCapture?.(e.pointerId);
  };
  const movePan = (e) => {
    const pan=panRef.current; if(!pan || pan.pointerId!==e.pointerId) return;
    e.preventDefault();
    const focusX=clamp(pan.startFocusX-((e.clientX-pan.startX)/pan.width)*100,0,100);
    const focusY=clamp(pan.startFocusY-((e.clientY-pan.startY)/pan.height)*100,0,100);
    onUpdatePhoto(pan.photoId,{focusX,focusY});
  };
  const endPan = (e) => { if(panRef.current?.pointerId===e.pointerId) panRef.current=null; };
  const setZoom = (photo, value) => onUpdatePhoto(photo.id,{zoom:clamp(Number(value),1,3)});
  const resetPhoto = (photo) => onUpdatePhoto(photo.id,{zoom:1,focusX:50,focusY:50});
      ...
      return <div key={idx} style={frameStyle} className={`photo-slot slot-${idx} ${editor?'editor-slot':''} ${editor&&selectedPhoto?'tap-ready':''} ${activeSlot===idx?'adjusting':''} ${page.storyMode?'free-scrap-photo':''}`} onDragOver={editor?(e)=>e.preventDefault():undefined} onDrop={editor?(e)=>onDrop(e,idx):undefined} onClick={editor&&selectedPhoto?()=>onTapSlot(idx):undefined}>
        {photo ? <>
          ...
          <div className={`photo-adjust-tools ${activeSlot===idx?'visible':''}`} onPointerDown={(e)=>e.stopPropagation()} onClick={(e)=>e.stopPropagation()}>
            <button type="button" title="Zoom out" onClick={()=>setZoom(photo,adjust.zoom-.1)} disabled={adjust.zoom<=1.001}><ZoomOut size={14}/></button>
            <input aria-label="Photo zoom" type="range" min="1" max="3" step="0.05" value={adjust.zoom} onChange={(e)=>setZoom(photo,e.target.value)}/>
            <button type="button" title="Zoom in" onClick={()=>setZoom(photo,adjust.zoom+.1)} disabled={adjust.zoom>=2.999}><ZoomIn size={14}/></button>
